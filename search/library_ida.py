...
def ida(initial_state):
    ...
    while True:
        ...
        NODES = {str(initial_state) :{f["path"]:"",f["depth"]:path_level,f["move"]:"", f["parent"]:""}}  # path and depth of the path
        n = len(initial_state)
        value = False
            ...
            if status.checkState():
                ...
                for node in nodes:
                    if str(node[0]) not in NODES: 
                        NODES[str(node[0])] = {f["path"]:"",f["depth"]:path_level,f["move"]:node[1],f["parent"]:str(state)}
